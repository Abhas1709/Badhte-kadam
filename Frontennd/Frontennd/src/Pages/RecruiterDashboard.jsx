...
export default function RecruiterDashboard() {
    ...
    const [chatInfo, setChatInfo] = useState({ open: false, jobId: null, applicantId: null });
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex">
                ...
                <div className="absolute bottom-0 w-full p-4 border-t border-gray-100">
                    <div className="flex items-center gap-3 mb-4 px-2">
                        <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 font-bold">
                            {profile?.companyName?.[0] || "R"}
                        </div>
                        <div className="overflow-hidden">
                            <p className="text-sm font-medium text-gray-900 truncate">{profile?.companyName || "Recruiter"}</p>
                            <p className="text-xs text-gray-500 truncate">{profile?.email}</p>
                        </div>
                    </div>
                    {myUserId && (
                        <div className="px-2 mb-3">
                            <Link to={`/recruiter/public/${myUserId}`} className="text-xs text-gray-500 hover:underline">
            ...
            <main className="flex-1 md:ml-64 p-8">
                                                            ...
                                                            <div key={app.applicant.id} className="bg-white p-4 rounded-lg border border-gray-200 flex justify-between items-center">
                                                                ...
                                                                <div className="flex gap-2">
                                                                    ...
                                                                    {(app.status === "shortlisted" || app.status === "accepted" || app.status === "hired") && (
                                                                        <button
                                                                            onClick={() => setChatInfo({ open: true, jobId: selectedJob, applicantId: app.applicant.id })}
                                ...
                                <div className="p-6 space-y-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-2">
                                                <Link to={`/student/public/${viewApplicant.applicant.id}`} className="text-indigo-600 hover:underline text-sm">
                                                    View Public Profile
                                                </Link>
                                            </div>
                                        </div>
                ...
                {chatInfo.open && <ChatWindow jobId={chatInfo.jobId} applicantId={chatInfo.applicantId} onClose={() => setChatInfo({ open: false, jobId: null, applicantId: null })} />}
